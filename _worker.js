...
async function generateTrojanLinksFromSource(list, user, userDomain, disableNonTLS, customPath, echConfig) {
  const links = [];
  list.forEach(item => {
    let nodeNameBase = item.isp ? item.isp.replace(/\s/g, '_') : (item.name || item.domain || item.ip);
    const safeIP = item.ip.includes(':') ? `[${item.ip}]` : item.ip;
...
function generateVMessLinksFromSource(list, user, userDomain, disableNonTLS, customPath, echConfig) {
  const links = [];
  list.forEach(item => {
    let nodeNameBase = item.isp ? item.isp.replace(/\s/g, '_') : (item.name || item.domain || item.ip);
    const safeIP = item.ip.includes(':') ? `[${item.ip}]` : item.ip;
...
function generateLinksFromNewIPs(list, user, userDomain, customPath, echConfig) {
  return generateLinksFromSource(list, user, userDomain, true, customPath, echConfig);
}
