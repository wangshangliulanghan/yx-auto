...
// =====================
const EDGE_CACHE_TTL_HOME = 60 * 60 * 6;        // 主页缓存 6 小时
const EDGE_CACHE_TTL_SUB = 60 * 5;              // 订阅缓存 5 分钟（可按需改 60~600）
const EDGE_CACHE_TTL_UPSTREAM = 60 * 10;        // 外部数据缓存 10 分钟
const EDGE_CACHE_SWR_HOME = 60 * 60;            // 主页 stale-while-revalidate 1 小时
const EDGE_CACHE_SWR_SUB = 60 * 10;             // 订阅 stale-while-revalidate 10 分钟
const EDGE_CACHE_SWR_UPSTREAM = 60 * 10;        // 上游 stale-while-revalidate 10 分钟

function withCacheHeaders(resp, sMaxAge, swr = 0) {
  const r = new Response(resp.body, resp);
  // 浏览器不缓存（max-age=0），边缘缓存（s-maxage）
  r.headers.set(
    "Cache-Control",
...
// 请求优选API（CDN缓存优化：缓存上游返回的原始文本 10分钟）
...
// 从GitHub获取优选IP（CDN缓存优化：缓存 txt 文本 10分钟）
...
async function handleSubscriptionRequest(request, user, customDomain, piu, ipv4Enabled, ipv6Enabled, ispMobile, ispUnicom, ispTelecom, evEnabled, etEnabled, vmEnabled, disableNonTLS, customPath, echConfig = null) {
  ...
  // 注意：这里原本是 no-store；为不动业务逻辑，我们在路由入口处统一覆盖 Cache-Control（见 fetch handler）
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (path === '/' || path === '') {
      ...
      let resp = new Response(generateHomePage(scuValue), {
    ...
    if (pathMatch) {
      // CDN缓存优化：按完整 URL 缓存最终订阅结果（短缓存 + SWR）
      ...
      let resp = await handleSubscriptionRequest(
